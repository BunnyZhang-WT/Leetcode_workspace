...
public:
    int myAtoi(string s) {
        ...
        while (i < s.length())
        {
            ...
            {
                s[i] = s[i] - 48;
                if (output > INT_MAX / 10 || (output == INT_MAX / 10 && s[i] > INT_MAX % 10))
                    return INT_MAX;
                else if (output < INT_MIN / 10 || (output == INT_MIN / 10 && -s[i] < INT_MIN % 10))
                    return INT_MIN;
                output = output * 10 + flag*s[i];
            }
            else
                return output;
            ++i;
